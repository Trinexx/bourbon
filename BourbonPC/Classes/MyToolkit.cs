...
class MyToolkit
{
    ...
    public static bool InstallDirSafe(string path) {
        try {
            File.Move(Path.Combine(path, "Bourbon.exe"), Path.Combine(path, "Bourbon_rename.exe"));
            if (File.Exists(Path.Combine(path, "Bourbon_rename.exe")))
            {
                File.Move(Path.Combine(path, "Bourbon_rename.exe"), Path.Combine(path, "Bourbon.exe"));
                return true;
            } else {
                return false;
            }
    ...
    public static void ActivityLog(string Line) {
        try
        {
            using (StreamWriter writer = new StreamWriter(Path.Combine(Bourbon.Settings.GamePath, "BourbonActivityLog.txt"), true))
            {
